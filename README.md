...
適用於不需要真實資料庫、只需要檔案持久化加記憶體快取與簡易搜尋的專案。
資料持久化至磁碟（AOF + JSON 快取），同時在記憶體中保留一份快取加速讀取，萬筆資料約佔 5~50 MB（視單筆 JSON 大小而定）。
適合資料量在萬筆以內、單機單進程的場景：
...
**不適合**：高併發線上服務（>1000 QPS）、資料量超過記憶體、跨機器存取、多進程同時寫入。

專案本質是 KV 儲存，搜尋能力透過值的型別判斷實現，非傳統資料庫引擎。後續將提供匯出至 MongoDB 的工具函式，資料結構天然相容。
